...
## From Session 5 (signature layer)

13. **Signature layer is partially shipped — S2 only.** Session 5 shipped the project-spring token + the S2 daypart clock (the flagship) and stopped for Mike's phone feel-check (the gate). The rest of S1–S4 is scoped in docs/BASELINE.md's Session 5 entry: S1 arrival ring / section rule / save ring-draw / map-pulse retune, S3 drone imagery (DEFER off /today — only ~2.5KB byte headroom; put it on /collections + /plan), S4 layoutId card→detail, and the S2 copy shift. Build these only after Mike approves the daypart direction, so nothing layers on an unapproved foundation.
14. **A live daypart cross-fade needs a client tick.** The daypart is computed server-side and lands on the first render after sunset passes (app routes revalidate often). A mid-session live cross-fade (e.g. the user sits on the page through sunset) would need a small client island reusing the existing @property color-interpolation pattern. Deferred to v2.
