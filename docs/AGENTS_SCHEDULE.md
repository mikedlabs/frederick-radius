...
| Weekly Mon 13:13 UTC                                                       | **Firecrawl Source Watch**    | Checks the exact reviewed County Connector schedule page. It creates review signals only and reserves four or five of 30 monthly credits. Downtown Frederick events and the County food-truck roster stay manual until each passes its own two-run proof.   | GitHub Actions (`.github/workflows/source-intelligence.yml`)       |
...
- **Source Watch, Source Scout, and the Apify radar** run outside the visitor
  request path and produce review queues only. Their exact schedule selects
  tracked profiles or source IDs, initializes no missing state, and cannot
...
| `FIRECRAWL_API_KEY`                    | Secret   | Source Watch in the GitHub `Production` environment                           | Retrieves one exact reviewed page per scheduled run.                                      |
...
The scheduled Source Watch does not enable the extraction fallbacks. Keep
