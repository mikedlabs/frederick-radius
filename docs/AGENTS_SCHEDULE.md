...
| -------------------------------------------------------------------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Twice hourly at :04 and :34                                                | `event-archive`               | Reuses the hot event-cache products to upsert durable event identities and aliases. It records its own strict heartbeat, can be retried safely through idempotent batches, and only tombstones rows from publishers whose complete raw inventory succeeded. | Vercel cron (`/api/cron/event-archive`)                                  |
...
| Dormant                                                                    | `parking-alerts`              | The alert route remains ready, but no licensed live occupancy feed is configured. Leaving it out of `vercel.json` avoids a no-op function invocation every ten minutes.                                                                                      | Manual activation only after feed verification                           |
| Dormant                                                                    | `saved-reminders`             | Device-scoped reminders remain ready but are not enabled in Production. Leaving the route unscheduled avoids a no-op function invocation every ten minutes until the end-to-end reminder flow is deliberately activated.                                    | Manual activation only after owner verification                          |
...
| Dormant                                                                    | `business-status`             | The legacy status-only diagnostic is manual. The scheduled hours refresh already collects Google business status in the same paid Place Details request, so a second sweep would pay twice for the same observation.                                      | Manual diagnostic only; not present in `vercel.json`                     |
...
| Every 2 hours at :05                                                       | `data-health-feeds`           | Pulls the bounded live-feed set and persists one current snapshot for each successful source.                                                                                                                                                               | Vercel cron (`/api/cron/data-health-feeds`)                              |
| Every 2 hours at :43                                                       | `runtime-source-health`       | Records the compact runtime-source heartbeat view separately from full feed snapshots.                                                                                                                                                                      | Vercel cron (`/api/cron/runtime-source-health`)                          |
| Dormant                                                                    | `data-health-retention`       | The bounded retention route remains inert and unscheduled until a current database backup is confirmed and `DATA_RETENTION_PRUNE=1` is deliberately enabled.                                                                                              | Manual activation only after backup verification                         |
...
Each GitHub Actions workflow listed above exposes `workflow_dispatch` for a
manual run. All cron times are UTC; Frederick County
...
to run unattended: `hours-refresh` collects hours and business status in one
cost-capped Google request, `data-health` snapshots freshness, `ingest/civicengage`,
...
- **data-steward** pulls the reviewed hours and business-status snapshot,
  rebuilds the place release, and opens a **PR** so the data diff is
  reviewed before it merges. It never pushes straight to a content
  branch. The core hours snapshot pull fails loudly when its
...
| -------------------------------------- | -------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `GOOGLE_PLACES_API_KEY`                | Secret   | a deliberately confirmed live `discover` run                                  | Google Places lookup for reviewed place discovery; the Vercel hours writer uses its own Production secret. |
...
| `ANTHROPIC_BUSINESS_INFO_API_KEY`      | Secret in `Data Enrichment` | business-information extraction | Isolates the business extractor from the other scheduled model jobs. |
| `ANTHROPIC_CIVIC_API_KEY`              | Secret in `Data Enrichment` | municipal civic extraction | Isolates the civic extractor and its provider usage. |
| `ANTHROPIC_VENUE_EVENTS_API_KEY`       | Secret in `Data Enrichment` | venue-event extraction | Isolates venue refresh usage and failures. |
...
positive `RADIUS_SEARCH_EMBEDDING_DAILY_DOCUMENT_LIMIT`, `OPENAI_API_KEY`, and
the verified migration `0017` usage counter. A key alone never enables them.
Vercel OIDC and AI Gateway authenticate Ask Radius text generation. Although
Gateway offers embedding models, this scheduled writer currently uses the
direct OpenAI provider and does not inherit the Ask text-generation transport.
Its optional
