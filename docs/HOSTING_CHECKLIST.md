...
| `GOOGLE_PHOTO_DAILY_CAP=25`                                | Vercel | Shared daily ceiling for the photo proxy and photo-health probes. The default stays below the current 1,000-request monthly free allowance even in a 31-day month; the application cannot accept a value above 100.                       |
| `GOOGLE_PLACE_ENRICH_DAILY_CAP=10`                         | Vercel | Shared daily ceiling for basic and user-requested current-hours enrichment. The application cannot accept a value above 80.                                                                                                               |
| `GOOGLE_PLACE_EXPERIENCE_DAILY_CAP=5`                      | Vercel | Shared daily ceiling for deliberate experience-detail requests. The application cannot accept a value above 20.                                                                                                                          |
...
| `HOURS_REFRESH_CRON=0`                                     | Vercel | Paid catalog-wide hours refresh stays off by default. Enable only after reviewing the eligible catalog, Google quota, and forecast; stale schedules remain safely withheld while it is off.                                                 |
...
| `BUSINESS_STATUS_CRON=0`                                   | Vercel | Paid rotating closure-status checks stay off by default. Run a bounded, reviewed snapshot manually when the data steward needs one.                                                                                                       |
