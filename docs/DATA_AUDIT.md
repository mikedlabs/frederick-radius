...
- **Unified event set** — `src/lib/loaders/unifiedEvents.ts` — THE assembly (curated seeds + iCal live + Ticketmaster + Bandsintown + venue lineups), deduped/classified/time-sanity-guarded — kept hot by `/api/cron/warm-events` every 15 minutes. Durable identities are written separately by `/api/cron/event-archive` every two hours so an archive retry cannot consume the visitor-cache warm budget.
