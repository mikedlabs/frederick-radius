...
- **Ingest crons (DB-persisting)** — `/api/ingest/civicengage` (daily 09:00: Frederick County and municipal CivicEngage calendars → `raw_events` + user-facing `ingested_events`, with per-domain `ingest_runs` heartbeats), `/api/ingest/fcpl` (09:15, Frederick County Public Libraries, `src/lib/ingest/fcpl.ts`), `/api/ingest/fcvfra` (09:20, fire/rescue association events, `src/lib/ingest/fcvfra.ts`); plus manual routes `all`, `arcgis`, `celebrate`, `county`, `seed`. Cache-key rule: bump `ingested-series-vN` after shape changes (PR #509).
