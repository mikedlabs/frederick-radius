...
echo "Browser runner bootstrap started."
...
if [ "$(id -u)" = "0" ]; then
  echo "Preparing isolated browser runner state."
...
cd "$runner_root"
echo "Unprivileged browser runner user is ready."

mkdir -p "${HOME:-$runner_root/home}" "${XDG_CACHE_HOME:-$runner_root/cache}" "${npm_config_cache:-$runner_root/cache/npm}"

runner_url="${RUNNER_URL:-https://github.com/mikedlabs/frederick-radius}"
runner_name="${RUNNER_NAME:-radius-browser-nas}"
runner_labels="${RUNNER_LABELS:-radius-browser}"
...
if [ ! -f .runner ]; then
  ...
  echo "Registering the NAS browser runner with GitHub."
  ./config.sh \
    --unattended \
    --url "$runner_url" \
    --token "$RUNNER_TOKEN" \
