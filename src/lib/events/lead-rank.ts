...
const ROUTINE_PROGRAM =
  /\b(story\s?times?|lap\s?sit|toddler time|baby (time|lap)|preschool (story|play)|tutor(ing)?|study (hall|group)|homework help|knit(ting)?|crochet|cross\s?stitch|quilt(ing)?|craft (time|club|circle)|coloring|chess club|game (club|night at the library)|book club|reading group|writers? (group|club)|\bguild\b|water\s?color|yoga|tai chi|zumba|pilates|meditation|mindfulness|support group|grief group|english conversation|\besl\b|citizenship class|computer (class|basics|help)|tech (help|tutor)|résumé|resume help|job (club|help|seekers)|genealogy|drop[- ]?in (craft|play|tech|help))\b/i;
...
export function eventLeadTier(e: EventWithMeta): number {
...
export function compareForLead(a: EventWithMeta, b: EventWithMeta): number {
...
export function pickLeadEvent(pool: EventWithMeta[]): EventWithMeta | null {
