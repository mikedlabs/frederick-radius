...
const dormant: Pick<ParkingPin, "available" | "percentFull" | "isFull" | "isFilling"> = {
  available: null,
  percentFull: null,
  isFull: false,
  isFilling: false,
...
describe("parkingSpacesLabel — the honest no-number rule", () => {
  it("shows a real count when the feed has one", () => {
    expect(parkingSpacesLabel({ available: 42, isFull: false })).toBe("42 spaces open");
  });

  it("uses the singular for exactly one space", () => {
    expect(parkingSpacesLabel({ available: 1, isFull: false })).toBe("1 space open");
  });

  it("says Full when the deck is full", () => {
    expect(parkingSpacesLabel({ available: 0, isFull: true })).toBe("Full");
  });

  it("returns null (NO number) when availability is unknown", () => {
    expect(parkingSpacesLabel({ available: null, isFull: false })).toBeNull();
  });

  it("Full wins over a null count", () => {
    expect(parkingSpacesLabel({ available: null, isFull: true })).toBe("Full");
  });
});
