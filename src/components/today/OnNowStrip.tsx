...
function OnNowChipCard({ chip }: { chip: OnNowChip }) {
  return (
    <Link
      ...
      className="tactile-interactive flex min-h-[44px] max-w-[15rem] flex-col justify-center gap-0.5 rounded-[var(--app-radius-md)] px-3 py-2"
      style={{
    ...
    >
      <span
        ...
        style={{ color: chip.kind === "event" ? "var(--app-brand-press)" : "var(--app-ink-3)" }}
      >
        {chip.kind === "event" && (
      ...
      <span className="truncate font-serif text-[14px] font-semibold leading-tight tracking-[-0.01em]" style={{ color: "var(--app-ink)" }}>
        {chip.title}
      </span>
      {chip.meta && (
        <span className="truncate text-[11.5px] leading-snug" style={{ color: "var(--app-ink-2)" }}>
          {chip.meta}
        </span>
      )}
    </Link>
