 ...
 *
 * The full roster is long, so the default wall shows a scannable first screen
 * and reveals the rest on tap; a filtered set (already short) shows in full
 * once the count is at or under the cap.
 */
...
export default function BeerMosaic({
  ...
  const visible = showAll ? beers : beers.slice(0, INITIAL_TILES);
  const hidden = beers.length - visible.length;
    ...
    <>
              ...
              >
                ...
                <span className="flex flex-1 flex-col gap-0.5 p-2">
                  <span className="flex items-start gap-1">
                    ...
                    <span className="font-serif text-[13.5px] font-semibold leading-tight text-[#281e14] line-clamp-1">{b.name}</span>
                  </span>
      ...
      {hidden > 0 && (
        <button
          ...
          className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-full border border-black/12 bg-[#faf5ea] px-4 text-[13px] font-semibold text-[#5e3a15] transition active:scale-[0.99]"
        >
          Show all {beers.length} beers
        </button>
      )}
    </>
  );
