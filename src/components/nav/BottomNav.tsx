...
import { Sun, Map, Calendar, Bookmark, MoreHorizontal } from "lucide-react";
import { haptic } from "@/lib/haptics";
import MoreSheet from "./MoreSheet";

// Primary destinations. Pre-launch review §7: human labels over
// internal slugs. The /browse route was relabeled to "Map" since
// the spatial view (real map at the top, intent chips, place list)
// is what users open the tab for — "Browse" was abstract. Route
// stays /browse so every deep link, bookmark, and shared URL in
// the wild still resolves.
//
// The fifth tab, "More", doesn't navigate — it opens a bottom
// drawer with every secondary route.
const TABS = [
  { href: "/now",    label: "Today",  icon: Sun,             fillOnActive: true,  kind: "link"   as const },
  { href: "/browse", label: "Map",    icon: Map,             fillOnActive: false, kind: "link"   as const },
  { href: "/events", label: "Events", icon: Calendar,        fillOnActive: false, kind: "link"   as const },
  { href: "/saved",  label: "Saved",  icon: Bookmark,        fillOnActive: true,  kind: "link"   as const },
  { href: "#more",   label: "More",   icon: MoreHorizontal,  fillOnActive: false, kind: "drawer" as const },
] as const;
