...
export default function FairUpNext({ targetDate = new Date(Date.now() + 30 * 60000) }: { targetDate?: Date }) {
  const { timeLeft, isUrgent } = useCountdown(targetDate);
        ...
        >
          ...
          <div className="p-4 sm:p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border bg-[var(--app-bg-surface)] shadow-sm outline-none transition-all hover:bg-[var(--app-bg-elevated)] focus-visible:ring-2 focus-visible:ring-[var(--app-brand)] active:scale-90"
                style={{ borderColor: "var(--app-border)" }}
                aria-label="Star this event"
              >
                <Star className="h-5 w-5 text-yellow-500 drop-shadow-sm transition-transform hover:scale-110" />
              </button>
            </div>
            <div className="mt-5">
              <Button
