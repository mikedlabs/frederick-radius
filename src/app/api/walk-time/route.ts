 ...
 * real streets from the (o)rigin to the (d)estination. This upgrades
 * the map's directions chip from the straight-line WALKING_MPS estimate
 * (which can't see Carroll Creek, the railroad tracks, or a one-way
 * grid) to an honest figure.
 *
    ...
    return Response.json(
      {
        ...
        meters: Number.isFinite(route.distance) ? Math.round(route.distance as number) : null,
      },
      {
        headers: {
