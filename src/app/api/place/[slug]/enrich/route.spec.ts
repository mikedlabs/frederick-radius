...
vi.mock("@/lib/google-photo-policy", () => ({ publishableGooglePhotoNames: vi.fn(() => []) }));
...
function request(mode?: "hours" | "experience") {
  return new Request(
...
describe("place enrichment shared daily budgets", () => {
  ...
  it("uses the shared basic allowance for a current-hours check", async () => {
    await GET(request("hours"), params);
  ...
  it("returns one coherent live-hours result after visitability review", async () => {
    const structured = {
    ...
    mocks.getPlaceDetails.mockResolvedValue({
      weekday_hours: ["Monday: Open 24 hours"],
      photo_names: [],
      photo_attributions: [],
      business_status: "OPERATIONAL",
    });
    mocks.parseGoogleHours.mockReturnValue(structured);
    mocks.mayPublishVisitabilityHours.mockReturnValue(true);

    const response = await GET(request("hours"), params);
    const body = await response.json();

    expect(body.hours).toEqual(["Monday: Open 24 hours"]);
    expect(body.structured_hours).toEqual(structured);
    expect(body.open_status?.state).toMatch(/open|closed|closing-soon/);
    expect(Date.parse(body.hours_checked_at)).not.toBeNaN();
  });
  ...
  it.each([null, { reserved: false, count: 30 }])(
